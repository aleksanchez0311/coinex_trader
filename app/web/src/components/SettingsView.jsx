    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="glass p-8">
        ...
        <div className="flex flex-wrap gap-3">
          <button 
            ...
            className="bg-surface border border-accent text-accent font-bold px-6 py-2.5 rounded-lg hover:bg-accent/10 transition-colors"
          >
            Restaurar Local
          </button>
          <button 
            onClick={() => saveCredentials(credentials)}
            className="bg-accent text-black font-bold px-6 py-2.5 rounded-lg hover:opacity-90 transition-opacity"
          >
          ...
          <button 
            ...
            className="bg-transparent border border-short text-short font-bold px-6 py-2.5 rounded-lg hover:bg-short/10 transition-colors ml-auto"
          >
            Borrar
          </button>
        </div>
      ...
      <div className="glass p-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="flex flex-col border border-border/40 rounded-xl bg-surface/30 h-[500px]">
             <div className="p-4 border-b border-border/20">
              ...
              <div className="relative">
                ...
                <input 
                  type="text" 
                  placeholder="Buscar símbolo (ej: SHIB, DOT)..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full bg-surface border border-border p-2.5 pl-10 rounded-lg outline-none focus:border-accent transition-all text-sm"
                />
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-2 scrollbar-premium">
              {(activeTab === 'all' && allMarkets.length === 0) || (activeTab === 'gainers' && topGainers.length === 0) ? (
                <div className="p-10 text-center space-y-3">
                  ...
                  <p className="text-sm text-gray-500">Cargando mercados...</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 gap-1">
                  {displayedMarkets.length > 0 ? (
                    displayedMarkets.map((m) => {
                      ...
                      return (
                        <div key={m.symbol} className="flex items-center justify-between p-2.5 hover:bg-accent/5 rounded-lg transition-colors group">
          ...
          <div className="flex flex-col border border-border/40 rounded-xl bg-surface/30 h-[500px]">
            <div className="p-4 border-b border-border/20 bg-white/5">
              <h3 className="font-bold text-sm flex items-center justify-between">
                Tus Favoritos Actuales
                <span className="text-xs text-accent bg-accent/10 px-2 py-0.5 rounded-full">{favorites.length}</span>
              </h3>
              <p className="text-xs text-gray-400 mt-1">Estos pares aparecerán en el lateral de tu Dashboard.</p>
            </div>
            <div className="flex-1 overflow-y-auto p-2 scrollbar-premium">
              {favorites.length === 0 ? (
                ...
                <div className="grid grid-cols-1 gap-1">
                  {favorites.map(symbol => (
                    <div key={symbol} className="flex items-center justify-between p-3 bg-surface border border-border rounded-lg group">
