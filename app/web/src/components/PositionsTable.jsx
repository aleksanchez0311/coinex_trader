...
const PositionsTable = ({ credentials }) => {
  ...
  return (
    <div className="glass overflow-hidden border-border/40">
      <div className="p-4 border-b border-border/40 flex justify-between items-center bg-white/5">
            ...
            {positions.length === 0 ? (
              ...
              positions.map((pos, idx) => (
                <tr key={idx} className="hover:bg-white/5 transition-colors group">
                  ...
                  <td className="px-4 py-4 font-mono">{pos.contracts}</td>
                  <td className="px-4 py-4 font-mono text-gray-400">{pos.entryPrice?.toFixed(2)}</td>
                  <td className="px-4 py-4 font-mono text-gray-400">{pos.markPrice?.toFixed(2)}</td>
                  <td className={`px-4 py-4 font-mono font-bold ${
                    pos.unrealizedPnl >= 0 ? 'text-long' : 'text-short'
                  }`}>
                    {pos.unrealizedPnl >= 0 ? '+' : ''}{pos.unrealizedPnl?.toFixed(2)}
                  </td>
                  <td className={`px-4 py-4 font-bold flex items-center gap-1 ${
                  ...
                  <td className="px-4 py-4 text-center">
                    <button 
                      ...
                      className="px-3 py-1 bg-short/10 text-short border border-short/30 rounded hover:bg-short hover:text-white transition-all font-bold text-[10px] disabled:opacity-50 disabled:grayscale"
                    >
